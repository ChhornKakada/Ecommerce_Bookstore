...
export default {
  setup() {
    ...
    // use 'ref' if you want to modify the value in the template
    const isPaypal = ref(true);

    const yearRange = computed(() => {
      const range = 5;
      const startYear = currentYear.value - range;
      const endYear = currentYear.value + range;
      return Array.from({ length: endYear - startYear + 1 }, (_, index) => startYear + index);
    });

    return {
      currentYear, months, isPaypal, yearRange, currentYear, checkoutStore
    };
  }
};
</script>
...
<template>
  <nav class="w-full flex gap-6">
    <button @click="isPaypal = true"
      class="w-2/5 focus:bg-[#292D3E] flex justify-center bg-white border-[#003087] py-2 border-2">
    ...
    <button @click="isPaypal = false"
      class="w-2/5 bg-white focus:bg-[#292D3E] py-2 border-2 border-[#003087] flex justify-center items-center gap-4">
  ...
  <div class="w-full text-[1.3rem] font-bold py-4">Payment Details</div>
  <!-- paypal -->
  <form action="" v-if="isPaypal">
    <input type="email" required class="mt-4 w-full py-3 border border-black pl-4" placeholder="Email" />
    <input type="number" required class="mt-4 w-full py-3 border border-black pl-4" placeholder="Card Number" />
  </form>

  <!-- credit card -->
  <form action="" v-else>
    <input type="text" required class="mt-4 w-full py-3 border border-black pl-4" placeholder="Cardholder Name" />
    <input type="number" required class="mt-4 w-full py-3 border border-black pl-4" placeholder="Card Number" />
    <div class="w-full flex gap-4 mt-4">
      <select name="month" id="month" class="w-1/3 border border-black pl-4">
        <option class="text-[#9CA3AF]">Month</option>
        <option v-for="month in months" :key="month" :value="month">{{ month }}</option>
      </select>
      <select name="year" id="year" class="w-1/3 border border-black pl-4 py-3">
        <option class="text-[#9CA3AF]">Year</option>
        <option v-for="year in yearRange" :key="year" :value="year">{{ year }}</option>
      </select>
      <input type="number" required class="w-1/3 py-3 border border-black pl-4" placeholder="CVV" />
    </div>
  ...
  <button class="text-center mt-8 w-full text-white tracking-wider bg-[#0D0D0D] py-4" @click="checkoutStore.test">
    Pay with card
  </button>
</template>
