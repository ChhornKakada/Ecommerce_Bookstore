      ...
      for (let i = 0; i < this.cart.length; i++) {
        if (this.cart[i].quantity.bookId == bookId) {
          if (this.cart[i].quantity.qty + qty >= 1) {
            ...
            this.cart[i].quantity.totalPrice = this.cart[i].quantity.qty * this.cart[i].price;
          }
          break;
        }
      }
...
<template>
  ...
  <div class="w-[80%] mx-auto mb-6 min-h-[100vh]">
    <h1 v-if="checkoutStore.state === 'discount'" class="text-[2.2rem] font-bold tracking-wider mb-4">Your Cart</h1>

    <div class="xl:flex">
      <!-- left -->
      <div class="xl:w-1/2">
        <h1 v-if="checkoutStore.state !== 'discount'" class="text-[1.5rem] mt-12 font-bold tracking-wider mb-4">Your Cart
        </h1>
                ...
                <div class="grow flex-col justify-end">
                  <div>
                    ...
                    <div class="flex items-center gap-3">
                      ...
                      <button @click="increaseBookQty(book.quantity.bookId, -1, book.price)">
      ...
      <div class="xl:pl-36 xl:w-1/2 xl:mb-0 mb-6">
        <Discount v-if="checkoutStore.state === 'discount'" :totalPrice="getTotalPrice" />
        <div v-else class="text-[1.2rem] font-medium ">
          <div class="w-full text-[2rem] font-bold tracking-wider">Checkout</div>
          <div class="flex items-center gap-4 mt-4 ">
            <button :class="{ selected: checkoutStore.state === 'address' }" @click="checkoutStore.goToCheckout"
              class="hover:underline">Address</button>
            <div class="grow">
              <div class="w-full h-[2px] bg-black"></div>
            </div>
            <button :class="{ selected: checkoutStore.state === 'shipping' }" @click="handleShpping"
              class="hover:underline">Shipping</button>
            <div class="grow">
              <div class="w-full h-[2px] bg-black"></div>
            </div>
